...
@section('title', 'Tambah Service | Admin Panel')
@push('styles')
<style></style>
@endpush
@section('content')
...
<nav class="flex items-center gap-1.5 text-[12px] text-gray-400 mb-3" aria-label="Breadcrumb">
    ...
    <a  href="{{ route('layanans.index') }}" class="hover:text-brand transition-colors">Layanan</a>
    <i class="ti ti-chevron-right text-[10px]"></i>
    <span class="text-gray-600 font-medium">Tambah Layanan</span>
</nav>

<!-- Judul halaman -->
<div class="flex items-center justify-between mb-5">
    <h1 class="text-[22px] font-bold text-gray-800">Add New Service</h1>
        ...
        <form action="{{ route('layanans.store') }}" method="POST" enctype="multipart/form-data" class="p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-gray-500">Format yang didukung: MP3, WAV, OGG, AAC, WMA. Maksimal 10MB.</p>
...
<script>
    ...
    document.querySelector('form').addEventListener('submit', function(e) {
        if (!confirm('Apakah Anda yakin ingin menyimpan data service ini?')) {
            e.preventDefault();
        }
    });
</script>
